...
src/
- components/ — shared components (`PageHeader`, `SearchBox`, `FilterBar`, `ConfirmationDialog`, `EmptyState`/`LoadingState`/`ErrorState`, `StatusTag`, `StatCard`, `DevSwitcher`, `Logo`, etc.), plus `components/advertiser/` — a deliberately separate, scoped subtree for the Advertiser portal's own orange/charcoal design system (its own `theme.ts`, `StatusChip`, `EmptyState`/`LoadingState`/`ErrorState`). Never import across that boundary — see the design-system skill.
- pages/ — one file per route (`Homepage`, `AdvertiserDashboard`, `AdminDashboard`, `DriverDashboard`, `LoginPage`, `SignupPage`)
- layouts/ — page-shell wrappers: `DashboardShell` (Admin), `MobileShell` (Driver), `AuthLayout` (Login/Signup). The Advertiser portal's shell (`AdvertiserLayout`) intentionally stays under `components/advertiser/` instead, to keep it colocated with its scoped design system.
...
- types/ — one file per domain (`advertiser.ts`, `admin.ts`, `driver.ts`)
- data/ — mock fixture data matching the `types/` shapes (`advertiserMockData.ts`, `adminMockData.ts`, `driverMockData.ts`) — this stands in for a real API until a backend exists; see below
