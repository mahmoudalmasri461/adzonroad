...
import type { Car, CarStatus } from '../../types/taxiCompany';
        ...
        {cars.map((car) => {
          ...
          return (
            <CircleMarker
              ...
              pathOptions={{
                color: '#fff',
                fillColor: STATUS_COLOR[car.status],
                fillOpacity: 1,
                weight: isSelected ? 3 : 2,
              }}
              eventHandlers={{ click: () => setSelectedId(car.id) }}
            />
          );
        })}
      </MapContainer>
      ...
      {selected && (
        ...
        >
          ...
          <Box sx={{ display: 'grid', gap: '5px', fontSize: 12 }}>
            <Row label="Screen ID" value={selected.screenId} />
            <Row label="Model" value={`${selected.model} (${selected.year})`} />
            <Row label="Status" value={selected.status} color={STATUS_COLOR[selected.status]} />
            <Row label="GPS" value={selected.gpsStatus} />
